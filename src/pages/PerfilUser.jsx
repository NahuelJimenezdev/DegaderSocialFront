  ...
  useEffect(() => {
    const cargarPublicaciones = async () => {
      try {
        ...
        const response = await fetch("http://localhost:3001/api/publicaciones/usuario", {
    ...
    if (!profile) {
      ...
      (async () => {
        try {
          ...
          const { data } = await apiFetch(buildApiUrl(API_CONFIG.ENDPOINTS.ME), {
            signal: ac.signal,
          });
        ...
        <div className="px-4 px-md-5 pb-4 pb-md-5">
          <div className="d-flex align-items-end gap-3" style={{ marginTop: -64 }}>
            <div
              ...
              style={{ width: 128, height: 128, background: "linear-gradient(135deg,#60a5fa,#fbbf24)" }}
            >
              {avatar_url ? (
                <>
                  <img
              ...
              <div className="d-flex align-items-start justify-content-between">
                <div>
                  ...
                  <div className="d-flex align-items-center gap-2">
                    <span className={`badge rounded-pill ${statusBadgeClass} text-capitalize`}>{status}</span>
                    <span className={`badge rounded-pill ${roleBadgeClass} text-capitalize`}>{role}</span>
                    <span className="badge rounded-pill bg-primary-subtle text-primary-emphasis text-capitalize">Nivel {hierarchy_level}</span>
                  </div>
                </div>

                <button className="btn btn-primary d-flex align-items-center gap-2" onClick={() => setOpenEdit(true)}>
                  <Edit size={16} />
        ...
        <div className="col-lg-8 d-flex flex-column gap-4">
          ...
          {/* <div className="container my-4" style={{ maxWidth: "64rem" }}>
            <div className="alert alert-warning d-flex align-items-center mb-3">
              ...
              <small>
                <strong>Modo offline:</strong> Las publicaciones se guardan localmente. Se sincronizarán cuando el backend esté disponible.
              </small>
            </div>
          </div> */}
                ...
                <div key={publicacion._id} className="card mb-3">
                  <div className="card-body">
                    <div className="d-flex align-items-center gap-2 mb-2">
                      <img
                        src={
                          publicacion.autor?.fotoPerfil ? `http://localhost:3001${publicacion.autor.fotoPerfil}` : "/default-avatar.png"
                        }
                      ...
                      <div>
                        <strong>
                          {publicacion.autor?.primernombreUsuario} {publicacion.autor?.primerapellidoUsuario}
                        </strong>
                    ...
                    {publicacion.titulo && <h6 className="card-title">{publicacion.titulo}</h6>}
                    ...
                    <div className="d-flex gap-3 mt-3">
                      <button className="btn btn-sm btn-outline-primary">Me gusta ({publicacion.likes?.length || 0})</button>
                      <button className="btn btn-sm btn-outline-secondary">Comentar ({publicacion.comentarios?.length || 0})</button>
                    </div>
                  </div>
                </div>
              ))
            ...
            <div className="card-body">
              ...
              <div className="d-flex flex-column gap-3">
                ...
                <div className="d-flex align-items-center justify-content-between">
                  ...
                  <span className="fw-semibold">{profile.grupos?.length || 0}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="card shadow-sm border-0">
            <div className="card-body">
              ...
              <div className="d-grid gap-2">
                <button className="btn btn-light text-start">Ver mis publicaciones</button>
                <button className="btn btn-light text-start">Gestionar carpetas</button>
                <button className="btn btn-light text-start">Configuración de privacidad</button>
              </div>
            </div>
          </div>
